...
type AddRankingModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (item: { name: string; link: string }) => void;
};

export const AddRankingModal: React.FC<AddRankingModalProps> = ({ isOpen, onClose, onAdd }) => {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[24px] w-full max-w-md flex flex-col overflow-hidden shadow-2xl">
        <div className="pt-6 pb-4 px-6 flex justify-between items-center border-b border-gray-100">
          <h2 className="font-bold text-[20px] text-[#ea92be]">Adicionar ao Ranking</h2>
