...
type EditAcessorioModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onEdit: (item: { name: string; image: string; link: string }) => void;
  item: { name: string; image: string; link: string } | null;
};

export const EditAcessorioModal: React.FC<EditAcessorioModalProps> = ({ isOpen, onClose, onEdit, item }) => {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[24px] w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
        <div className="pt-6 pb-4 px-6 flex justify-between items-center border-b border-gray-100">
          <h2 className="font-bold text-[20px] text-[#ea92be]">Editar Acessório</h2>
