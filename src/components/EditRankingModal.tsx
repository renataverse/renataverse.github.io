...
type EditRankingModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onEdit: (item: { name: string; link: string }) => void;
  item: { name: string; link: string } | null;
};

export const EditRankingModal: React.FC<EditRankingModalProps> = ({ isOpen, onClose, onEdit, item }) => {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[24px] w-full max-w-md flex flex-col overflow-hidden shadow-2xl">
        <div className="pt-6 pb-4 px-6 flex justify-between items-center border-b border-gray-100">
          <h2 className="font-bold text-[20px] text-[#ea92be]">Editar Ranking</h2>
        ...
        <div className="px-6 py-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Nome do Livro/Série *</label>
            <input 
              type="text" 
              value={name}
